        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    config_->enableVideoStream(OB_STREAM_COLOR, 480, 270, 60, OB_FORMAT_RGB);
                    color_configured = true;
                    std::cout << "[DEBUG] COLOR configured: 480x270@60fps ✓" << std::endl;
                } catch (const ob::Error& e2) {
                    std::cout << "[WARNING] COLOR 480x270@60fps failed: " << e2.what() << std::endl;
            ...
            try {
                ...
                if (!colorProfile) {
                    std::cout << "[WARNING] No color profile found, using default" << std::endl;
            ...
            // Search for 480x270 depth profile first, then fallback
            try {
                ...
                for (uint32_t i = 0; i < depthProfiles->getCount(); i++) {
                    ...
                    if (profile->getWidth() == 480 && profile->getHeight() == 270) {
