...
private:
    ...
    // OpenCV windows
    cv::Mat m_depthDisplay;
    cv::Mat m_colorDisplay;
    cv::Mat m_irDisplay;
    cv::Mat m_rgbdDisplay;
    
public:
    RGBDIRViewer() : m_handle(nullptr), m_bDeviceOpen(false), m_bGrabbing(false), m_frameCount(0) {
    }
        ...
        for (unsigned int i = 0; i < frameData.nImageCount; i++) {
            ...
            if (!displayMat.empty()) {
                ...
                std::string frameInfo = "Frame: " + std::to_string(imageData.nFrameNum) + 
                                       " | Size: " + std::to_string(imageData.nWidth) + "x" + std::to_string(imageData.nHeight) +
                                       " | Type: " + std::to_string(imageData.enImageType);
                
                cv::putText(displayMat, frameInfo, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
    ...
    int RunViewer() {
        ...
        std::cout << "Starting real-time viewer..." << std::endl;
        std::cout << "Press 'q' on any window or ESC to quit." << std::endl;
