...
void showPointCloud(const pcl::PointCloud<pcl::PointXYZ>& pclPointCloud)
{
	...
	pclCloudViewer.addPointCloud(pclPointCloud.makeShared());
...
int main(int argc,char** argv)
{
    ...
    if (MV3D_RGBD_OK == nRet)
    {
        if (0 == stFrameData.nValidInfo)
        {
            ...
            {
                // ch:转换PCL格式点云数据 | en:Convert PCL format point cloud data
                pcl::PointCloud<pcl::PointXYZ> pclPointCloud(pstPointCloudImage->nWidth, pstPointCloudImage->nHeight);
                convertToPCL(*pstPointCloudImage, pclPointCloud);
                // ch:保存PCL点云文件 | en:Save PCL format point cloud file
                char chFileName[256] = { 0 };
                sprintf(chFileName, "[%d]_PointCloud.ply", pstPointCloudImage->nFrameNum);
                pcl::PLYWriter pclWriter;
                pclWriter.write(chFileName, pclPointCloud, false);
                LOGD("Save PCL format point cloud success!");
                // ch:渲染点云 | en:Display point cloud
