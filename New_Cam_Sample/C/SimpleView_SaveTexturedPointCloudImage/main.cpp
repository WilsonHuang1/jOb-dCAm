...
int main(int argc,char** argv)
{
    ...
    while (!bExit_Main)
    {
        ...
        if (MV3D_RGBD_OK == nRet)
        {
            ...
            MV3D_RGBD_IMAGE_DATA* pstTexture = NULL;
            for (int i = 0; i < stFrameData.nImageCount; i++)
            {
                ...
                {
                    LOGD("Get rgb8 planar image succeed: FrameNum[%d] Width[%d] Height[%d] Len[%d] CoordinateType[%d]!", stFrameData.stImageData[i].nFrameNum,
                        stFrameData.stImageData[i].nWidth, stFrameData.stImageData[i].nHeight, stFrameData.stImageData[i].nDataLen, stFrameData.stImageData[i].enCoordinateType);
                    pstTexture = &stFrameData.stImageData[i];
                }
				if (ImageType_YUV422 == stFrameData.stImageData[i].enImageType)
				{
					LOGD("Get YUV422 image succeed: FrameNum[%d] Width[%d] Height[%d] Len[%d] CoordinateType[%d]!", stFrameData.stImageData[i].nFrameNum,
						stFrameData.stImageData[i].nWidth, stFrameData.stImageData[i].nHeight, stFrameData.stImageData[i].nDataLen, stFrameData.stImageData[i].enCoordinateType);
					pstTexture = &stFrameData.stImageData[i];
				}
            }
            // ch:保存纹理点云图像 | en: Save textured point cloud image
            if (NULL != pstPointCloudImage && NULL != pstTexture)
            {
                char chFileName[256] = { 0 };
                sprintf(chFileName, "[%d]_TexturedPointCloudImage", pstPointCloudImage->nFrameNum);
                nRet = MV3D_RGBD_SaveTexturedPointCloudImage(handle, pstPointCloudImage, pstTexture, PointCloudFileType_PLY, chFileName);
                if (MV3D_RGBD_OK == nRet)
                {
