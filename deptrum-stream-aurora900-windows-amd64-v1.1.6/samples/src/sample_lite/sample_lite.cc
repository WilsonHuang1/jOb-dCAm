...
int NebulaSetCalibrationMode(std::shared_ptr<Device> device) {
  ...
  int32_t mode{0};
  std::cin >> mode;
  auto ret = std::dynamic_pointer_cast<Nebula200>(device)->SetCalibrationMode(mode);
...
int PrepareDevice() {
  ...
  // std::dynamic_pointer_cast<Nebula200>(device)->SetMtofCropPixels(0, 0);
#endif
...
#endif
  } else if (device_name == "Nebula200" || device_name == "Nebula220 mipi" ||
